...
- `palette256-zlib`(`flags=2`)은 장면에서 고른 256색을 쓴다. 계약은 `INTERFACE.md` §12.3이다.
  ...
  - **적합도가 나빠지면 다시 고른다.** 매 Frame 표본 픽셀에서 "고른 팔레트 색 ↔ 실제 색"의
    RGB 거리를 재고(`paletteFitError`), `PALETTE_REFIT_ERROR`(12)를 넘으면 재계산한다.
    히트맵이 안개처럼 깔리면 dBm 설정은 그대로여도 카메라가 움직이는 것만으로 시야의
    dBm 값이 바뀌어 화면 색 구성이 달라진다. 이벤트로는 잡히지 않는다.
    실측: 팔레트를 고른 그 화면 2.7 / 살짝 이동 5.5 / 중간 이동 24.2 / 많이 이동 54.0.
    최소 간격은 `PALETTE_REFIT_INTERVAL`(3초)이다.
  - **계산은 별도 Thread에서 돈다.** 그동안 송신은 직전 팔레트로 계속되므로 재계산이
    잦아져도 화면이 멈추지 않는다.
  - 마찬가지로 **heatmap On/Off, 방식, dBm 범위가 바뀌면 다시 고른다.** 화면에 없던 색이
