...
void sibr::GaussianView::onRenderIBR(sibr::IRenderTarget & dst, const sibr::Camera & eye)
{
	...
	// Gaussian 결과 위에 RF Volume을 덧그린 뒤 그 화면 그대로를 송신한다.
	if (_rfVolume)
	{
	...
	if (_streamer)
	{
		if (_rfVolume)
		{
			_streamer->overlay(_rfVolume->methodName(), _rfVolume->dbmRange().x(),
				_rfVolume->dbmRange().y(), _rfVolume->enabled());
		}
		_streamer->capture(dst);
	}
}
