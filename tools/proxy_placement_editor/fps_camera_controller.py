...
class FpsNavigationSettings:
    ...
    def from_dict(cls, value: Dict) -> "FpsNavigationSettings":
        return cls(
            ...
            horizontal_only=bool(value.get("horizontal_only", True)),
        )
...
def movement_basis(
    forward: np.ndarray, right: np.ndarray, horizontal_only: bool = True
    ...
    return _normalized(forward, "Camera forward"), _normalized(
        right, "Camera right"
    )
...
class FpsCameraController:
    ...
    def step(self, view_matrix, now: float, sprint: bool = False) -> np.ndarray:
        ...
        forward, right = movement_basis(
            forward, right, horizontal_only=self.settings.horizontal_only
        )
