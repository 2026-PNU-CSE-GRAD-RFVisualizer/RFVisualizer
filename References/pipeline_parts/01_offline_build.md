...
PGSR 메시의 큰 평면을 단순한 Proxy Geometry로 바꾸는 현재 도구는 일반 평면 추출과 벽 전용 추출을 분리한다. 벽 전용 경로는 일반 추출의 잔여점에 의존하지 않고, 같은 전처리 원본에서 점 법선이 수직면에 가까운 점만 골라 별도 RANSAC을 실행한다. 이 결과는 벽 후보를 회수하는 단계이며, 방 외곽 생성이나 모서리 맞춤은 이후 단계로 남긴다.
