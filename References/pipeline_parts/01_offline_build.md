...
현재 Proxy Mesh Editor에는 실제 변환 전 사전 진단 단계가 있다. 닫힌 Room Envelope에서 `scene.up_vector`의 부호, 모든 바닥·천장 대응점의 높이, `+Z` 정렬 순수 회전, 단일 배율 후보, 원점과 X축 후보를 계산한다. 이 단계의 사진 기반 문 크기는 임시 추정치이며, 실제 미터 변환은 현장 실측값으로 교체한 뒤 수행한다. 바닥 법선은 계단형 바닥의 경사를 포함할 수 있으므로 위쪽 기준으로 강제 사용하지 않는다.
